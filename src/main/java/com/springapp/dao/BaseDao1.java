...
public class BaseDao1<T> {
    ...
    public void delete(int id) {
        Session session = getSession();
               JdbcTransaction tx = session.getTransaction().getLocalStatus()==LocalStatus.ACTIVE?(JdbcTransaction)session.getTransaction():(JdbcTransaction) session.beginTransaction(); 
        try {
            session.delete(session.get(getClz(), id));
            tx.commit();
        } catch (Exception err) {
            if (tx != null) {
                tx.rollback();
    ...
    public T get(Long id) {
        ...
        T entity = null;
        try {
            entity = (T) session.get(getClz(), id);
            tx.commit();
        } catch (Exception err) {
            if (tx != null) {
                tx.rollback();
    ...
    public T load(int id) {
        ...
        Session session = getSession();
        JdbcTransaction tx = (JdbcTransaction)session.getTransaction();         if(tx.getLocalStatus().equals(LocalStatus.ACTIVE))             System.out.print("已存在活动的事务");         else             tx=(JdbcTransaction) session.beginTransaction();
        T entity = null;
        try {
            entity = (T) session.load(getClz(), id);
            tx.commit();
        } catch (Exception err) {
            if (tx != null) {
                tx.rollback();
    ...
    public Object query1(String hql,int max) {
        Session session = getSession();
               JdbcTransaction tx = session.getTransaction().getLocalStatus()==LocalStatus.ACTIVE?(JdbcTransaction)session.getTransaction():(JdbcTransaction) session.beginTransaction(); 
        hql = initSort(hql);
        Query query = session.createQuery(hql);
    ...
    public List<T> list4Chinese(String hql,int max) {
        Session session = getSession();
               JdbcTransaction tx = session.getTransaction().getLocalStatus()==LocalStatus.ACTIVE?(JdbcTransaction)session.getTransaction():(JdbcTransaction) session.beginTransaction(); 
        hql = initSort(hql);
        Query query = session.createQuery(hql);
